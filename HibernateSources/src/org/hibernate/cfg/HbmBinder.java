...
/**
 * XML映射文件作为跳板，生成Hibernate配置时元模型 (the classes in the <tt>mapping</tt> package)
 *
 * @author Gavin King
 */
public final class HbmBinder {
	...
	/**
	 *  hbm.xml映射文件解析并绑定，使用给定的DOM执行必要的绑定操作
	 *
	 * @param doc 等待解析和绑定DOM
	 * @param mappings 当前绑定的状态
	 * @param inheritedMetas 任何继承meta标记信息.
	...
	public static void bindRoot(Document doc, Mappings mappings, java.util.Map inheritedMetas)
		...
		Element hmNode = doc.getRootElement();//根节点<hibernate-mapping>
		// get meta's from <hibernate-mapping>
		inheritedMetas = getMetas( hmNode, inheritedMetas, true );
		extractRootAttributes( hmNode, mappings );//提取根节点的属性值(7个)

		Iterator rootChildren = hmNode.elementIterator();//获取根节点的所有子节点
		while ( rootChildren.hasNext() ) {
			...
			if ( "filter-def".equals( elementName ) ) {
				...
				bindRootClass( element, rootclass, mappings, inheritedMetas );//绑定RootClass
	...
	/**
	 * 负责执行hbm.xml映射文件中&lt;class/&gt;元素的绑定操作
	 *
	 * @param node DOM&lt;class/&gt;元素.
	 * @param rootClass 存储绑定信息的mapping实例
	 * @param mappings 当前绑定的状态
	 * @param inheritedMetas 任何继承meta-tag信息.
	 * @throws MappingException
	 */
	 ...
	 * 
	 * @param node <class>节点
	 * @param persistentClass 持久类
	 * @param mappings 存放 配置信息
	 * @param inheritedMetas 继承元数据
	 * @throws MappingException
	 */
	public static void bindClass(Element node, PersistentClass persistentClass, Mappings mappings,
		...
		// 传递一个明确定义的实体名称
		// 处理lazy属性
		Attribute lazyNode = node.attribute("lazy");//懒加载属性
		boolean lazy = lazyNode == null ?
				mappings.isDefaultLazy() :
				"true".equals(lazyNode.getValue());//默认lazy为true
		// 继续向前,这里设置懒加载, 后来pojo.proxy可以覆盖它.
		persistentClass.setLazy( lazy );

		String entityName = node.attributeValue( "entity-name" );//获取entity-name属性值
		if ( entityName == null ) entityName = getClassName( node.attribute("name"), mappings );//获取<name>属性值做为实体名称
		if ( entityName==null ) {
			throw new MappingException( "Unable to determine entity name" );
		}
		persistentClass.setEntityName( entityName );//设置实体名称
	...
	private static void bindPojoRepresentation(Element node, PersistentClass entity,
		...
		String className = getClassName( node.attribute( "name" ), mappings );//获取完整类名
		String proxyName = getClassName( node.attribute( "proxy" ), mappings );//获取完整代理类名
	...
	private static Element locateTuplizerDefinition(Element container, EntityMode entityMode) {
		Iterator itr = container.elements( "tuplizer" ).iterator();//查找<class/>节点中所有的<tuplizer/>节点
		while( itr.hasNext() ) {
			final Element tuplizerElem = ( Element ) itr.next();//获取tuplizer节点
	 ...
	 * 获取实体名称 
	 * 
	 * @param elem &lt;entity-name&gt;节点
	 * @param model mappings集合
	 * @return
	 */
	...
	/**
	 * 返回 defaultPackage.unqualifiedName 包名.类名 做为 实体名
	 * 
	 * @param unqualifiedName 类名
	 * @param defaultPackage 默认包名
	 * @return
	 */
