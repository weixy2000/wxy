...
/**
 * 不同数据库之间复制数据行时，决定在和数据库中已存在记录有冲突时，replicate()如何处理
 *
...
public abstract class ReplicationMode implements Serializable {
	 ...
	 */
	static {
		INSTANCES.put( LATEST_VERSION.name, LATEST_VERSION );//取最新版本
		INSTANCES.put( IGNORE.name, IGNORE );//忽略
		INSTANCES.put( OVERWRITE.name, OVERWRITE );//覆盖
		INSTANCES.put( EXCEPTION.name, EXCEPTION );//抛出异常
	}
