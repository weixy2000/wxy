...
public class XmlConfigurationProvider implements ConfigurationProvider {
    ...
    public void init(Configuration configuration) {
        ...
        loadDocuments(configFileName);// 加载所有struts配置文件
    }
    ...
    public void register(ContainerBuilder containerBuilder, LocatableProperties props) throws ConfigurationException {
        ...
        for (Document doc : documents) {
            Element rootElement = doc.getDocumentElement();// 根元素
        ...
        for (Document doc : documents) {
            ...
            for (int i = 0; i < childSize; i++) {
                ...
                if (childNode instanceof Element) {
                    ...
                    if ("package".equals(nodeName)) {// package标签
    ...
    protected void addAction(Element actionElement, PackageConfig.Builder packageContext) throws ConfigurationException {
        ...
        // methodName如果没有设置应该是null
        methodName = (methodName.trim().length() > 0) ? methodName.trim() : null;

        // 如果没有给<action>标签指定className属性则尝试使用当前<package>下的default-class-ref
        if (StringUtils.isEmpty(className)) {// className为空
        	// 如果该<package>存在defaultClassRef则使用，否则使用ActionSupport
           /* if (StringUtils.isNotEmpty(packageContext.getDefaultClassRef())) {
            ...
            if (!verifyAction(className, name, location)) {// 验证action是否合法
    ...
    /**
     * 创建一个PackageConfig对象代表struts XML配置文件中package节点
     */
    protected PackageConfig addPackage(Element packageElement) throws ConfigurationException {
        PackageConfig.Builder newPackage = buildPackageContext(packageElement);

        if (newPackage.isNeedsRefresh()) {// 是否需要刷新
        ...
        // 加载当前package下的<result-type>标签
        addResultTypes(newPackage, packageElement);

        // 加载package中的<interceptor><interceptor-stack>拦截器标签
        loadInterceptors(newPackage, packageElement);

        // 加载当前package下的<default-interceptor-ref>标签
        loadDefaultInterceptorRef(newPackage, packageElement);

        // 加载当前package下的<default-class-ref>标签
        loadDefaultClassRef(newPackage, packageElement);

        // 加载当前package下的<global-results>标签
        loadGlobalResults(newPackage, packageElement);

        // 加载当前package下的<global-exception-mappings>标签
        ...
        // 加载当前package的<default-action-ref>标签
    ...
    protected void addResultTypes(PackageConfig.Builder packageContext, Element element) {
        ...
        for (int i = 0; i < resultTypeList.getLength(); i++) {
            ...
            String def = resultTypeElement.getAttribute("default"); // 是否是默认结果类型
    ...
    /**
     * 从给定的action节点中创建一个List&lt;InterceptorMapping&gt;集合;
     * @param element
     * @param context
     * @return
     * @throws ConfigurationException
    ...
    /**
     * 从给定的action节点中创建一个ResultConfig集合Map&lt;String, ResultConfig&gt;
     */
    protected Map<String, ResultConfig> buildResults(Element element, PackageConfig.Builder packageContext) {
        ...
        for (int i = 0; i < resultEls.getLength(); i++) {
            ...
            if (resultElement.getParentNode().equals(element) || resultElement.getParentNode().getNodeName().equals(element.getNodeName())) {
                ...
                // <result/>不指定name属性, 默认name值为"success"
                if (StringUtils.isEmpty(resultName)) {
                    resultName = Action.SUCCESS;
                }

                // 如果没有设置resultType,默认将会使用其父package的defaultResultType
                ...
                if (resultParams.size() == 0) // 也许我们仅仅存在一个body - 因此有一个默认的参数
                {
                	// 就像<result ...>something</result>这样我们最常使用的添加参数方式，添加了一个'something'参数
                    if (resultElement.getChildNodes().getLength() >= 1) {
                        resultParams = new LinkedHashMap<String, String>();

                        String paramName = config.getDefaultResultParam();// 参数name
                        if (paramName != null) {
                            ...
                            String val = paramValue.toString().trim();// 参数value
                            if (val.length() > 0) {
                ...
                // 创建一个新的param map, 以便于result param能够覆盖config param
    ...
    /**
     * 封装给定的&lt;global-exception-mappings&gt;节点下所有&lt;exception-mapping&gt;节点到ResultConfig对象map中
     */
    ...
    /**
     * 加载当前package下的&lt;default-action-ref&gt;标签
     * @param packageContext
     * @param element
     */
    ...
    /**
     * 从XML节点package中加载所有的&lt;global-results&gt;标签
     */
    ...
    /**
     * 从XML节点package中加载所有的&lt;default-class-ref&gt;标签
     * @param packageContext
     * @param element
     */
	...
	/**
	 * 从XML节点package中加载所有的&lt;global-exception-mappings&gt;标签
	 * @param packageContext
    ...
    //    }
    /**
     * 加载&ltinterceptor-stack&gt中的单个&lt;interceptor-ref&gt;
     */
    ...
    private List<Document> loadConfigurationFiles(String fileName, Element includeElement) {
        ...
        if (!includedFileNames.contains(fileName)) {
            ...
            includedFileNames.add(fileName);// 被包含的fileName集合
            ...
            for (Document doc : docs) {
                ...
                for (int i = 0; i < childSize; i++) {
                    ...
                    if (childNode instanceof Element) {
                        ...
                        if ("include".equals(nodeName)) {// <include>标签
                            String includeFileName = child.getAttribute("file");// 包含的文件名称
