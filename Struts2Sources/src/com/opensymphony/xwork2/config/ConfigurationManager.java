...
/**
 * 该类是XWork配置文件管理的核心类。
 * <br/>ConfigurationManager - central for XWork Configuration management, including its ConfigurationProvider.
 *
    ...
    public synchronized void conditionalReload() {
        if (FileManager.isReloadingConfigs()) {
            ...
            for (ContainerProvider provider : providers) {
                if (provider.needsReload()) {// 需要重新加载
            ...
            if (reload) {// 需要重新加载
            	for (ContainerProvider containerProvider : containerProviders) {
                	try {
                		containerProvider.destroy();// 先销毁
                	}
                	catch(Exception e) {
                		LOG.warn("error while destroying configuration provider ["+containerProvider+"]", e);
                	}
                }
                packageProviders = configuration.reloadContainer(providers);// 重新加载容器
            }
        }
    }
