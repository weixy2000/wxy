...
public class Dispatcher {
    ...
    /**
     * 存储DispatcherListener的集合实例
     */
    private static List<DispatcherListener> dispatcherListeners =
        new CopyOnWriteArrayList<DispatcherListener>();

    /**
     * 存储ConfigurationManager的实例, 在init方法中设置
     */
    private ConfigurationManager configurationManager;

    /**
     * 存储StrutsConstants类中STRUTS_DEVMODE属性设置的值
     */
    private boolean devMode;

    /**
     * 存储StrutsConstants类中STRUTS_I18N_ENCODING属性设置的值
     */
    private String defaultEncoding;

    /**
     * 存储StrutsConstants类中STRUTS_LOCALE属性设置的值
     */
    private String defaultLocale;

    /**
     * 存储StrutsConstants类中STRUTS_MULTIPART_SAVEDIR属性设置的值
     */
    private String multipartSaveDir;

    /**
     * 存储StrutsConstants类中STRUTS_MULTIPART_HANDLER属性设置的值
     */
    ...
    /**
     * 使用给定的ServletContext和初始化参数集合initParams创建一个Dispather实例
     *
     * @param servletContext 给定的servletContext
     * @param initParams 初始化参数集合
     */
    ...
    public void cleanup() {
        ...
        if (objectFactory instanceof ObjectFactoryDestroyable) {
            try {
                ((ObjectFactoryDestroyable)objectFactory).destroy();// 销毁
            }
            catch(Exception e) {
            	// 捕获任何在调用destory()方法期间可能出现的异常并用日志记录它
                LOG.error("exception occurred while destroying ObjectFactory ["+objectFactory+"]", e);
            }
        }

        // 为当前线程清除Dispatcher它自己
        instance.set(null);

        // 清除所有DispatcherListener
        ...
        // 清除所有interceptor通过调用它们的destroy()方法
     ...
     * <br/>加载配置文件，包括XML文件及零配置策略(注解),另外更新可选设置，包括是否重新加载配置文件及资源文件
     */
    public void init() {
    	// ConfigurationManager实例化
    	if (configurationManager == null) {
    		configurationManager = new ConfigurationManager(BeanSelectionProvider.DEFAULT_BEAN_NAME);
    	}

        try {
            init_DefaultProperties(); // [1]初始化默认属性配置文件
            init_TraditionalXmlConfigurations(); // [2]初始化传统的Xml配置文件
            init_LegacyStrutsProperties(); // [3]Struts2遗留的属性配置文件
            init_CustomConfigurationProviders(); // [5]初始化自定义ConfigurationProvider
            init_FilterInitParameters() ; // [6]初始化过滤器初始化参数
            ...
            if (!dispatcherListeners.isEmpty()) {// 存在Dispatcher监听器
                for (DispatcherListener l : dispatcherListeners) {
                    l.dispatcherInitialized(this);// 执行Dispatcher监听器初始化方法
                }
            }
        } catch (Exception ex) {
    ...
    /**
     * 将相关对象信息封装为Map（如：HttpServletRequest、Http parameters、HttpServletResponse、HttpSession、ServletContext、ActionMapping等对象信息），
     * 并存入到执行上下文Map中，返回执行上下文Map对象extraMap
     *
     ...
     * 包裹并返回给定的request或者直接返回给定的request对象
     * </p>
    ...
    public HttpServletRequest wrapRequest(HttpServletRequest request, ServletContext servletContext) throws IOException {
        // 已属于StrutsRequestWrapper的子类，不需要再次包裹
        if (request instanceof StrutsRequestWrapper) {
            return request;
        }

        String content_type = request.getContentType();// 内容类型
        if (content_type != null && content_type.indexOf("multipart/form-data") != -1) {// 文件上传表单
            MultiPartRequest mpr = null;
            //check for alternate implementations of MultiPartRequest 检查是否有MultiPartRequest类的替换类
            Set<String> multiNames = getContainer().getInstanceNames(MultiPartRequest.class);// 获取所有MultiPartRequest接口的实现类
            if (multiNames != null) {
                for (String multiName : multiNames) {
                    if (multiName.equals(multipartHandlerName)) {// 是否指定了文件上传的处理类
                        mpr = getContainer().getInstance(MultiPartRequest.class, multiName);// 是，指定了
                    }
                }
            }
            if (mpr == null ) {// 没有指定文件表单处理类，使用使用默认处理类
                mpr = getContainer().getInstance(MultiPartRequest.class);
            }
            request = new MultiPartRequestWrapper(mpr, request, getSaveDir(servletContext));// 文件上传RequestWrapper
        } else {
