...
public class DefaultActionMapper implements ActionMapper {
    ...
    */
    ...
    public ActionMapping getMapping(HttpServletRequest request,
        ...
        ActionMapping mapping = new ActionMapping();
        String uri = getUri(request);// 从request中获取uri

        int indexOfSemicolon = uri.indexOf(";");
        uri = (indexOfSemicolon > -1) ? uri.substring(0, indexOfSemicolon) : uri;
        // 从actionName中删除扩展名并将扩展名存储到mapping中供以后使用，返回无扩展名的actionName
        uri = dropExtension(uri, mapping);
        if (uri == null) {
            return null;
        }
        // 从uri中解析name和namespace
    ...
    protected ActionMapping parseActionName(ActionMapping mapping) {
        ...
        if (allowDynamicMethodCalls) {// 允许动态方法调用
            // 处理 "name!method" 命名约定.
            String name = mapping.getName();
            int exclamation = name.lastIndexOf("!");
            if (exclamation != -1) {
                mapping.setName(name.substring(0, exclamation));// 设置name
                mapping.setMethod(name.substring(exclamation + 1));// 设置method
            }
        }
        return mapping;
    }
    ...
    protected void parseNameAndNamespace(String uri, ActionMapping mapping,
        ...
        int lastSlash = uri.lastIndexOf("/");// 最后一个斜线
        if (lastSlash == -1) {
            namespace = "";
            name = uri;
        } else if (lastSlash == 0) {
            ...
            // 如果没有根结点的namespace，则返回默认的namespace为"/"
            namespace = "/";
            name = uri.substring(lastSlash + 1);
        } else if (alwaysSelectFullNamespace) {
        	// 简单地选择最后一个/之前的所有内容作为namespace
        ...
        mapping.setNamespace(namespace);// 设置namespace
        mapping.setName(name);// 设置name
    }
     ...
     * <br/>从actionName中删除扩展名并将扩展名存储到mapping中供以后使用，返回无扩展名的actionName
     *
     * @param name
     *            The action name
     * @param mapping The action mapping to store the extension in
     * @return 无扩展名的actionName
     */
    ...
    /**
     * 如果没有指定扩展名则返回null，否则返回默认的扩展名，List集合中第1个元素
     */
    ...
    public String getUriFromActionMapping(ActionMapping mapping) {
        ...
        if (mapping.getNamespace() != null) {// 命名空间
            uri.append(mapping.getNamespace());
            if (!"/".equals(mapping.getNamespace())) {
                uri.append("/");
            }
        }
        String name = mapping.getName();// actionName
        String params = "";
        if (name.indexOf('?') != -1) {
        ...
        if (null != mapping.getMethod() && !"".equals(mapping.getMethod())) {
            uri.append("!").append(mapping.getMethod());// 方法名
        }

        String extension = mapping.getExtension();// 扩展名
        if (extension == null) {
            extension = getDefaultExtension();
            // 如果context可用，查找当前的扩展名
        ...
        if (extension != null) {// 扩展名不为null

            if (extension.length() == 0 || (extension.length() > 0 && uri.indexOf('.' + extension) == -1)) {
                if (extension.length() > 0) {
                    uri.append(".").append(extension);// 扩展名
                }
                if (params.length() > 0) {
                    uri.append(params);// 参数
                }
            }
        }
