...
public class TableActions {
    ...
    public String getExportAction(String exportView, String exportFileName) {
    	...
    	//wxy修改如下代码使用户每次最多导出5000条数据
    	StringBuffer action = null;
    	Object obj = RetrievalUtils.retrieve(model.getContext(), "totalRows");
    	long totalRows = 0;
    	if (obj != null && obj instanceof Number) {
    		totalRows = Long.parseLong(obj.toString());
    		action = new StringBuffer("javascript:if("+totalRows+"<=5000){");
    	}else{
    		action = new StringBuffer("javascript:if(true){");
    	}
        ...
        //wxy加
        action.append("}else{alert('考虑到性能原因，本系统暂只支持每次导出5000条数据，请缩小查询条件！'); }");
